	...
	<div class="layui-fluid">
		<div class="layui-row">
			<form class="layui-form" method="post" action="add" id='form'>
				...
				<div class="layui-form-item">
					<label for="L_training" class="layui-form-label"> <span
						class="x-red">*</span>参加的培训
					</label>
					<div class="layui-input-inline">
						<select name="trianing_id" lay-search>
							<c:choose>
								<c:when test="${empty trainings }">
									<option value="0">暂无可选培训</option>
								</c:when>
								<c:otherwise>
									<c:forEach items="${trainings }" var="training">
										<option value="${training.id }">${training.trName }</option>
									</c:forEach>
								</c:otherwise>
							</c:choose>
						</select>
